...
void resetBoard() {
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            board[i][j] = ' '; // Mengubah angka menjadi spasi kosong
        }
    }
}
...
bool isDraw() {
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            // Jika masih ada spasi kosong, berarti belum seri
...
int choosePlayer() {
    ...
    // Header Tabel dengan kolom W, D, L
    cout << left << setw(3)  << "\n No" << " | " 
         << setw(16) << "Nama" << " | " 
         << setw(6)  << "Score" << " | " 
         << setw(6)  << "Games" << " | "
    ...
    // Menampilkan data setiap player dalam loop
    for (int i = 0; i < players.size(); i++) {
    ...
    // Output sesuai permintaan di gambar
    cout << "\nPlayer dipilih : " << players[p-1].nama << endl;
    cout << "\nSelamat datang kembali, " << players[p-1].nama << "!\n";
    
    // Menampilkan statistik detail player yang dipilih
    cout << "\nStatistik Anda :" << endl;
...
void playVsComputer(Player &p) {
    int result = 0; // Inisialisasi variabel result
    char ulang;
    do {
        resetBoard();
        displayGuide();
        ...
        p.totalGames++;
        showMatchResult(result); // Sekarang result sudah memiliki nilai 1, 2, atau 3
        showStatistics(p);
        savePlayers();
        pressEnter();
    ...
    do {
        ...
        while (true) {
            ...
            if (checkWin(turn)) {
                ...
                current.totalWins++; current.score += 3;
